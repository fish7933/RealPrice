# Project Summary
This project is an advanced freight cost calculator designed for logistics companies, aiming to optimize their operations by providing accurate shipping cost estimates based on various parameters such as ports of loading (POL), ports of discharge (POD), weight, and additional fees. Recent updates have enhanced SQL query handling for historical data, improved user experience, and introduced better error management. New features now allow for filtering and selecting historical dates in the cost calculation process.

# Project Module Description
The project consists of several functional modules:
- **CostCalculatorWithTabs**: Main component for calculating freight costs with a tabbed interface, now enhanced to support historical date input.
- **AdminDashboard**: Management interface for overseeing freight-related data, including agents and ports.
- **AgentSeaFreightTable**: Displays statistics for sea freight agents.
- **ShippingLineTable**: Manages details of shipping lines.
...
- **CostResultTable**: Displays results of optimized cost calculations.
- **CalculationHistory**: Manages historical calculation data.
- **SeaFreightDialog**: Dialog for selecting sea freight options, now with historical date support.
...
- **FreightSqlDialog**: Displays SQL queries related to freight calculations, now supporting historical dates.
- **CalculationSqlPreviewDialog**: Previews generated SQL queries based on user input, now using the historical date prop.
