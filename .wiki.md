# Project Summary
Shadcn-UI is a logistics and shipping web application designed to optimize freight management for businesses. It provides tools for cost calculation, historical data analysis, and user role management, utilizing Supabase for backend services. The project has recently enhanced user authentication, improved the Cost Calculator, and optimized deployment settings to support Single Page Application (SPA) routing.
...
- **CostCalculatorWithTabs**: Enhanced cost calculation interface with local charge input, recently updated to improve layout by moving domestic transport field to the same row as weight and DP.
