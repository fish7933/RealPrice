# Project Summary
Shadcn-UI is a logistics and shipping web application designed to optimize freight management for businesses. It provides essential tools for cost estimation, historical data analysis, and user role management, enhancing the overall efficiency of freight operations. The application utilizes Supabase for backend services, supporting robust user authentication and offering an advanced Cost Calculator. Recent enhancements have focused on improving component functionalities, such as a new magic button that allows users to view all freight options without constraints, thereby simplifying the decision-making process.
...
- **CostCalculator**: Tool for calculating shipping costs, enhanced with new features.
...
- **src/components/calculator/CostCalculatorWithTabs.tsx**: Updated component for cost calculation with tabs, now includes a magic button.
- **supabase/functions/reset_superadmin_password.js**: Edge Function for resetting the superadmin password.
- **vercel.json**: Updated deployment configuration for Vercel to fix 404 errors.
